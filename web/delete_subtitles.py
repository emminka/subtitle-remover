...
for opt, arg in opts:
    if opt in ['-a']:
        ...
        method_of_removing = int(arg)  
    elif opt in ['-i']:
        technique_of_removing = int(arg)  
    elif opt in ['-j']:
...
sum_progress_second = 700
sum_progress_not_keras = 0
sum_of_ones = 0
...
def gaussian(frame_s, maska): #method gaussian blur
    ...
    maska = cv2.GaussianBlur(maska, (5,5), 0) #blur mask
...
def not_zero_frame(subtitles_start):
    primary_mask = np.zeros((height_python,width_python),np.uint8) #vykreslenie ciernej masky v rozmeroch videa
    without_subtitles = subtitles_start - 1
    ...
    videocap.set(cv2.CAP_PROP_POS_FRAMES, without_subtitles)
    ...
    primary_mask[yL:yL+rows, xL:xL+cols] = cv2.addWeighted(primary_mask[yL:yL+rows, xL:xL+cols], 0, dilated, 1, 0)

    return primary_mask #put masks together
    ...
    if method_of_removing == 0:
        ...
        if ret == True:
            if(count_not_keras == 1 and fps_total < 950):
                ...
                wanted_every_x_frame  = everyframeislike*10
            ...
            if (count_not_keras ==helping_sum):
                repeat_every  = 950//(fps_total//wanted_every_x_frame )
                sum_progress_not_keras += repeat_every 
                print("PROGRESS: ",sum_progress_not_keras)
                helping_sum+=wanted_every_x_frame 
                sys.stdout.flush()
        ...
        if ret == True:
            ...
            if not found_subtitle_range:  #no subtitles, not changing frame
                ...
                if technique_of_removing == 0: #inpaiting NS
                    no_subtitles_frame = cv2.inpaint(frame,exact_mask,3,cv2.INPAINT_NS)
                    output.write(no_subtitles_frame)
                elif technique_of_removing == 1: #inpaiting telea
                    no_subtitles_frame = cv2.inpaint(frame,exact_mask,3,cv2.INPAINT_TELEA)
                    output.write(no_subtitles_frame)
                elif technique_of_removing == 2: #gauss
                    no_subtitles_frame = gaussian(frame,exact_mask)
                    output.write(no_subtitles_frame)
